...
Напишете програма, която по подаден път до папка, сканира тази папка на всеки 10 минути за файл с име `inputFlights.txt`. `inputFlights.txt` има произволен брой редове, всеки от които е в следния формат: `<origin> <dest> <priceInEuro>`.

- `origin` и `destination` са стрингове само от латински букви с дължина 3, а `priceInEuro` е цяло число.
- Данните от всеки ред трябва да се конкатенират във файл `outputFlights`, като двата стринга са същите, но всички малки букви са заместени с главни, а цената да е в лева (по курс 1.95).

**Примерен файл `inputFlights.txt`:**
