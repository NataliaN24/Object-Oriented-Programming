...
{
    return std::max(roundPowerOfTwo(stringLength + 1), 16u);
}
...
std::istream& operator>>(std::istream& is, MyString& ref)
{
    ...
    if (bufferStringSize > ref.getCapacity())
    {
        ref.resize(dataToAllocByStringLen(bufferStringSize));
    }
    strcpy(ref._data, buffer);
    ref._size = bufferStringSize;
    return is;
